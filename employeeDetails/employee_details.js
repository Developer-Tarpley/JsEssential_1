const employees = [
    ...
    { id: 4, name: 'Barry Son', age: 37, department: 'IT', salary: 70000, specialization: 'javaScript' },
    { id: 5, name: 'Derek Smokley', age: 40, department: 'Finance', salary: 65000,specialization: 'Accounting' },
    { id: 6, name: 'Katherine Hockles', age: 32, department: 'IT', salary: 80000, specialization: 'Java' },
...
function findEmployeeBySpecialization(specialization){
    const employeeSpecialization = employees.filter((employee=> employee.specialization === specialization));
    const foundSpecialization = employeeSpecialization.map((employee, index)=>{
        return `<p>${employee.id}: ${employee.name} - ${employee.department} - ${employee.specialization}</p>`;
    }).join('');
    document.getElementById('employeesDetails').innerHTML = foundSpecialization;
}